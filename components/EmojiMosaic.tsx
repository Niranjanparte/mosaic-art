...
interface MosaicCell {
  emoji: string; // Define the structure of each cell
}

interface EmojiMosaicProps {
  mosaic: MosaicCell[]; // Array of objects with `emoji` property
  gridSize?: number; // Optional grid size
  backgroundColor?: string; // Optional background color
}

export default function EmojiMosaic({ mosaic, gridSize = 128, backgroundColor = "black" }: EmojiMosaicProps) {
  ...
  return (
    <div className="flex m-4 flex-col items-center justify-center">
      ...
      <button
        className="bg-black px-4 py-3 rounded-md hover:text-gray-400 border-[1px] border-gray-400 m-4"
